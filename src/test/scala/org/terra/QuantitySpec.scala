...
class QuantitySpec extends FlatSpec with Matchers with CustomMatchers with TryValues {
  ...
  final class Thingee private (val value: Double, val unit: ThingeeUnit)
      extends Quantity[Thingee, Double, Tuple] {
  ...
  implicit object ThingeeNumeric 
      extends AbstractQuantityNumeric[Thingee, Tuple](Thingee)
  ...
  it should "create values using arbitrary numeric types" in {
    ...
    abstract class BaseNumeric[T] extends Numeric[T] {
      ...
      def negate(x: T) = -x
      def toInt(x: T) = toDouble(x).toInt
      def toLong(x: T) = toDouble(x).toLong
      def toFloat(x: T) = toDouble(x).toFloat
      def compare(x: T, y: T) = 
        if (toDouble(x) == toDouble(y)) 0 
        else if (toDouble(x) > toDouble(y)) 1 else -1
      def parseString(str: String): Option[T] = Some(str.asInstanceOf[T])
    }
  ...
  it should "create values from properly formatted Tuples with any numeric anyval" in {
    ...
    Thingee((10.22f, "th")).get should beApproximately[Thingee, Double, Tuple](Thangs(10.22))
    Thingee((10.22, "th")).get should be(Thangs(10.22))
  }
...
*/
  ...
  it should "plusOrMinus a like value and return a QuantityRange" in {
    val x = Thangs(5)
    val y = Thangs(1)
    (x plusOrMinus y) should be(QuantityRange[Thingee, Double, Tuple](Thangs(4), Thangs(6)))
    x +- y should be(QuantityRange[Thingee, Double, Tuple](Thangs(4), Thangs(6)))
  }
  ...
  it should "to a like value and return a QuantityRange" in {
    ...
    val y = Thangs(10)
    val r = x toThat y
    r should be(QuantityRange[Thingee, Double, Tuple](x, y))
  }
  ...
  it should "within a QuantityRange and return a Boolean" in {
    ...
    Thangs(10) within (Thangs(9) toThat Thangs(12)) should be(right = true)
  }
  ...
  it should "notWithin a QuantityRange and return a Boolean" in {
    ...
    Thangs(13) notWithin (Thangs(9) toThat Thangs(12)) should be(right = true)
  }
