...
import standard.energy.Watts
import standard.space.{ Meters, SquareMeters, SquareRadians }
...
class RadiantIntensitySpec extends FlatSpec with Matchers {
  ...
  it should "return Power when multiplied by SolidAngle" in {
    WattsPerSteradian(1) * SquareRadians(1) should be(Watts(1))
  }

  it should "return SolidAngle when divided by Power" in {
    WattsPerSteradian(1) / Watts(1) should be(SquareRadians(1))
  }
