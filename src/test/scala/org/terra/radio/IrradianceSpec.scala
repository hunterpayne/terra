...
import standard.energy.{ Watts, WattHours }
import standard.space.{ SquareCentimeters, SquareMeters }
import standard.time.Hours
...
class IrradianceSpec extends FlatSpec with Matchers with CustomMatchers {
  ...
  it should "return Energy when multiplied by AreaTime" in {
    implicit val tolerance = 0.000000000000001
  ...
  it should "provide aliases for single unit values" in {
    ...
    ergPerSecondPerSquareCentimeter should be(ErgsPerSecondPerSquareCentimeter(1))
  }
