...
class MoneyContextSpec 
  ...
  val moneyContext = MoneyContext(
    defCur, defaultMoneyContext.currencies, rates)
  ...
  it should "properly create Money using the default currency with an implicit MoneyContext in scope" in {
    implicit val moneyContext = 
      MoneyContext(defCur, defaultMoneyContext.currencies, rates)
  ...
  it should "return Some(rate) for a given currency pair for which there is no direct, but an indirect rate" in {
    implicit val tolerance: Double = 0.00000000000001
    moneyContext.indirectRateFor(XAG, XAU).get.rate.doubleValue should beApproximately(CurrencyExchangeRate(XAG(60), XAU(1)).rate.doubleValue)
  ...
  it should "properly return the same Money when converting to the same currency with an empty context" in {
    val context = MoneyContext(defCur, defaultMoneyContext.currencies, Nil)
  ...
  it should "throw a NoExchangeRateException when converting where an indirect rate exists but is not allowed by the context" in {
    val context = MoneyContext(
      defCur, defaultMoneyContext.currencies, 
      rates, allowIndirectConversions = false)
    intercept[NoSuchExchangeRateException] {
  ...
  it should "return a copy with additional currencies" in {
    ...
    implicit val context = moneyContext
    val NMY = Currency("NMY", "New Money", "$", 2)
    val OTM = Currency("OTM", "Other Money", "$", 2)
