...
import standard.space.{ Meters, Length }
import standard.time.{ Hours, Time }
 ...
 */
class MoneySpec 
    extends FlatSpec with Matchers with TryValues with CustomMatchers {

  behavior of "Money and its Units of Measure"
  implicit val moneyContext = defaultMoneyContext
  type PricePerTime = PricePer[Time]
  type PricePerLength = PricePer[Length]
  ...
  it should "create values using 'no currency' factories with an implicit MoneyContext in scope" in {
    implicit val moneyContext =
      MoneyContext(USD, defaultMoneyContext.currencies, Nil)
  ...
  it should "return proper result when comparing dislike currencies with a MoneyContext in scope" in {
    ...
    USD(10) equals JPY(1000) should be(right = true)
    USD(10) ==# JPY(1000) should be(right = true)
    USD(10) equals JPY(2000) should be(right = false)
    USD(10) ==# JPY(2000) should be(right = false)

    //USD(10) notEquals JPY(1000) should be(right = false)
    USD(10) !=# JPY(1000) should be(right = false)
    //USD(10) notEquals JPY(2000) should be(right = true)
    USD(10) !=# JPY(2000) should be(right = true)

    USD(1) compare JPY(100) should be(0)
    USD(1) compare JPY(1000) should be(-1)
    USD(1) compare JPY(10) should be(1)
  ...
  it should "return a proper result on max/min operation with an implicit MoneyContext in scope" in {
    ...
    implicit val moneyContext = 
      MoneyContext(USD, defaultMoneyContext.currencies, List(r1, r2))
    val x = USD(100)
    val y = JPY(100)
    x.max(y) should be(x)
    y.min(x) should be(y)
  ...
  it should "throw a UnsupportedOperationException when adding cross currency values with only the defaultMoneyContext in scope" in {
  ...
  it should "throw a UnsupportedOperationException when subtracting cross currency values with only the defaultMoneyContext in scope" in {
    intercept[UnsupportedOperationException] {
      USD(1) - JPY(2) should be(CAD(3))
    }
  }
  ...
  it should "throw a UnsupportedOperationException when dividing cross currency values with only the defaultMoneyContext in scope" in {
    intercept[UnsupportedOperationException] {
  ...
  it should "map over the underlying amount and return the resulting Money with no loss of precision" in {
    ...
    x.map(_ * 2) should be(USD(BigDecimal("44444444444444444444.44")))
  }
  ...
  it should "return a properly sorted list of Moneys" in {
    implicit val moneyContext = 
      MoneyContext(USD, defaultMoneyContext.currencies, Nil)
    val unorderedList = List[Money](Money(10), Money(100), Money(1))
  ...
  it should "provide implicit conversion from Double" in {
    ...
    d.XAU should be(Money(d, standard.market.XAU))
    d.USD should be(Money(d, standard.market.USD))
    d.dollars should be(Money(d, standard.market.USD))
    d.cents should be(Money(d / 100d, standard.market.USD))
    d.EUR should be(Money(d, standard.market.EUR))
    d.euros should be(Money(d, standard.market.EUR))
    d.JPY should be(Money(d, standard.market.JPY))
    d.yen should be(Money(d, standard.market.JPY))
    d.GBP should be(Money(d, standard.market.GBP))
    d.poundSterling should be(Money(d, standard.market.GBP))
    d.CHF should be(Money(d, standard.market.CHF))
    d.swissFrancs should be(Money(d, standard.market.CHF))
    d.AUD should be(Money(d, standard.market.AUD))
    d.CAD should be(Money(d, standard.market.CAD))
    d.SEK should be(Money(d, standard.market.SEK))
    d.NOK should be(Money(d, standard.market.NOK))
    d.NZD should be(Money(d, standard.market.NZD))
    d.BTC should be(Money(d, standard.market.BTC))
    d.bitCoin should be(BTC(d))
    d.ETH should be(Money(d, standard.market.ETH))
    d.ether should be(ETH(d))
    d.LTC should be(Money(d, standard.market.LTC))
  ...
  it should "provide Numeric support within a MoneyContext with no Exchange Rates" in {
    ...
    implicit val moneyNumeric = defaultMoneyContext.numeric
  ...
  it should "provide Numeric support within a MoneyContext with applicable Exchange Rates" in {
    ...
    implicit val moneyNumeric = moneyContext.numeric //new MoneyNumeric()
  ...
  it should "return price when dividing by quantity" in {
    ...
    (m /[Time, Double] t) should be(new PricePerTime(USD(100), Hours(1)))
  }

  it should "return quantity when dividing by price" in {
    val p = new PricePerLength(Money(10, "USD"), Meters(1))
    Money(40, "USD") / p should be(Meters(4))
  }

  it should "have a hashCode method" in {
    //implicit val moneyContext = defaultMoneyContext
    val j = new PricePerLength(Money(10, "JPY"), Meters(1))
    j.hashCode
    val p = new PricePerLength(Money(10, "USD"), Meters(1))
    p.hashCode
  }
