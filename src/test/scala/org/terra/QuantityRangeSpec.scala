...
import org.terra.thermal.TemperatureLike

import standard._
import standard.thermal.{ Celsius, Fahrenheit }
import standard.space.Meters
...
class QuantityRangeSpec extends FlatSpec with Matchers {
  ...
  object QuantityRangeT {
    def apply(
      lower: LengthLike[StandardTuple], upper: LengthLike[StandardTuple]): 
        QuantityRange[LengthLike[StandardTuple], Double, StandardTuple] =
      new QuantityRange[LengthLike[StandardTuple], Double, StandardTuple](
        lower, upper)
  }
  object QuantityRangeTemp {
    def apply(
      lower: TemperatureLike[StandardTuple], 
      upper: TemperatureLike[StandardTuple]):
        QuantityRange[TemperatureLike[StandardTuple], Double, StandardTuple] =
      new QuantityRange[TemperatureLike[StandardTuple], Double, StandardTuple](
        lower, upper)
  }

  it should "throw an IllegalArgumentException when the lower bound > upper bound" in {
    an[IllegalArgumentException] should be thrownBy QuantityRangeT(Meters(2), Meters(1))
  }

  it should "throw an IllegalArgumentException when the lower bound = upper bound" in {
    an[IllegalArgumentException] should be thrownBy QuantityRangeT(Meters(2), Meters(2))
  }

  it should "create a range with lower and upper bound" in {
    val r1 = QuantityRangeT(Meters(1), Meters(2))
    r1.lower should be(Meters(1))
    r1.upper should be(Meters(2))

    val r2 = QuantityRangeTemp(Celsius(0), Fahrenheit(100))
    r2.lower should be(Celsius(0))
    r2.upper should be(Fahrenheit(100))
  }

  it should "times a Double and get back a List of ranges" in {
    val r = QuantityRangeT(Meters(0), Meters(10))
    val rs = r times 3
    rs.head should be(QuantityRangeT(Meters(0), Meters(10)))
    rs.tail.head should be(QuantityRangeT(Meters(10), Meters(20)))
    rs.tail.tail.head should be(QuantityRangeT(Meters(20), Meters(30)))

    val rs2 = r * 3
    rs2.head should be(QuantityRangeT(Meters(0), Meters(10)))
    rs2.tail.head should be(QuantityRangeT(Meters(10), Meters(20)))
    rs2.tail.tail.head should be(QuantityRangeT(Meters(20), Meters(30)))
  }

  it should "divide a like value and get back a List of ranges" in {
    val r = QuantityRangeT(Meters(0), Meters(10))
    val rs = r / Meters(4)
    rs.head should be(QuantityRangeT(Meters(0), Meters(4)))
    rs.tail.head should be(QuantityRangeT(Meters(4), Meters(8)))
    rs.tail.tail.head should be(QuantityRangeT(Meters(8), Meters(10)))
  }
  ...
  it should "foreach a like value and execute an operation on each sub range" in {
    val r = QuantityRangeT(Meters(0), Meters(5))
    var accum = 0d
  ...
  it should "map a Double and applies a map operation on each sub range" in {
    val r = QuantityRangeT(Meters(0), Meters(35))
    val rs = r / 5
  ...
  it should "incFromDecTo a like value and return a QualityRange with lower and upper bounds incremented by the range value" in {
    (QuantityRangeT(Meters(0), Meters(10)) +- Meters(4)) should be(QuantityRangeT(Meters(4), Meters(6)))
  }
  ...
  it should "decFromIncTo a like value and return a QualityRange with lower and upper bounds incremented by the range value" in {
    (QuantityRangeT(Meters(0), Meters(10)) -+ Meters(4)) should be(QuantityRangeT(Meters(-4), Meters(14)))
  }

  it should "contains a like value and return true or false" in {
    !QuantityRangeT(Meters(0), Meters(10)).contains(Meters(-1)) should be(right = true)
    QuantityRangeT(Meters(0), Meters(10)).contains(Meters(0)) should be(right = true)
    QuantityRangeT(Meters(0), Meters(10)).contains(Meters(5)) should be(right = true)
    !QuantityRangeT(Meters(0), Meters(10)).contains(Meters(10)) should be(right = true)
    !QuantityRangeT(Meters(0), Meters(10)).contains(Meters(11)) should be(right = true)
  }

  it should "return equality" in {
    val r1 = QuantityRangeT(Meters(10), Meters(15))
    val r2 = QuantityRangeT(Meters(10), Meters(15))
    val r3 = QuantityRangeT(Meters(11), Meters(15))
  ...
  it should "contains a another range and return true or false" in {
    val r = QuantityRangeT(Meters(30), Meters(100))
    val rIn = QuantityRangeT(Meters(30), Meters(99))
    val rInOnEnd = QuantityRangeT(Meters(30), Meters(100))
    val rBeyondLower = QuantityRangeT(Meters(25), Meters(50))
    val rBeyondUpper = QuantityRangeT(Meters(50), Meters(110))
    val rBeyondBoth = QuantityRangeT(Meters(25), Meters(105))
    val rBelow = QuantityRangeT(Meters(10), Meters(20))
    val rOver = QuantityRangeT(Meters(105), Meters(205))
  ...
  it should "includes a like value and return true or false" in {
    !QuantityRangeT(Meters(0), Meters(10)).includes(Meters(-1)) should be(right = true)
    QuantityRangeT(Meters(0), Meters(10)).includes(Meters(0)) should be(right = true)
    QuantityRangeT(Meters(0), Meters(10)).includes(Meters(5)) should be(right = true)
    QuantityRangeT(Meters(0), Meters(10)).includes(Meters(10)) should be(right = true)
    !QuantityRangeT(Meters(0), Meters(10)).includes(Meters(11)) should be(right = true)
  }
