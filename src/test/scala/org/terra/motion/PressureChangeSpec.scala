...
class PressureChangeSpec extends FlatSpec with Matchers {
  ...
  it should "provide aliases for single unit values" in {
    ...
    pascalPerSecond should be(PascalsPerSecond(1))
    barPerSecond should be(BarsPerSecond(1))
    poundPerSquareInchPerSecond should be(PoundsPerSquareInchPerSecond(1))
    standardAtmospherePerSecond should be(StandardAtmospheresPerSecond(1))
  }
