...
import standard.mass.{KilogramsMetersSquared, PoundsSquareFeet}
import standard.space.{Feet, Meters}
...
class TorqueSpec extends FlatSpec with Matchers with CustomMatchers {
  ...
  it should "provide aliases for single unit values" in {
    ...
    newtonMeter should be(NewtonMeters(1))
    poundFoot should be(PoundFeet(1))
  }
