...
class RatioSpec extends FlatSpec with Matchers {
  ...
  case class TestRatio(base: Mass, counter: Length) 
      extends Ratio[Mass, Length, Double, StandardTuple]
  ...
  case class TestLikeRatio(base: Mass, counter: Mass) 
      extends LikeRatio[Mass, Double, StandardTuple]
