...
class InformationSpec extends FlatSpec with Matchers with CustomMatchers with TryValues {
  ...
  it should "properly convert to all supported Units of Measure" in {
    ...
    // TODO yetta and zetta level is broken due to the maximum precision of
    // a long, bits are also broken due to long not handling non-whole numbers

    implicit val tolerance: Double = 1.0
    val big = scala.math.pow(1000, 6)
    val x = Bytes(big)
    val x1 = Yottabytes(1)
    ...
    x.toExbibytes should be(1)
    //x1.toZebibytes should be(1024)
    ...
    //x1.toZebibits should be(8192)
    //x1.toYobibits should be(8)
  }
